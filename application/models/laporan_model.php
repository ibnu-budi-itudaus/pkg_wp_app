...
class laporan_Model extends CI_Model
{
    ...
    //     $this->db->join('alternatif', 'alternatif.id_alternatif=alternatif_kriteria.id_alternatif', 'left');
    //     $this->db->join('kriteria', 'kriteria.id_kriteria=alternatif_kriteria.id_kriteria', 'left');
    //     // $this->db->order_by('alternatif.id_alternatif');
    ...
    public function getdata($now)
    {
        ...
        $this->db->join('guru', 'guru.id_guru=laporan.id_guru', 'left');
        $this->db->join('alternatiff', 'alternatiff.id_alternatiff=laporan.id_alternatiff', 'left');
        $this->db->where('laporan.id_periode', $now);
    
        return $this->db->get()->result();
    }


    public function get_gurupkg($tahun)
    {
        $this->db->select('*');
        $this->db->from('laporan');
        $this->db->join('guru', 'guru.id_guru=laporan.id_guru', 'left');
        $this->db->join('hasil_pkg', 'hasil_pkg.id_guru=laporan.id_guru', 'left');
        $this->db->where('laporan.id_periode', $tahun);
         $this->db->order_by('laporan.nilai', 'DESC');
        return $this->db->get()->result();
    }

    public function get_AAllGuruu2($tahun)
    {
        // return $this->db->get_where('nilai', ['id_periode' => $tahun])->result_array();
        $query = "SELECT `id_kriteria`,`id_nilai`, `nama_guru`, `nip`, `nilai_guru`,`jabatan`, `jenis_kelamin`, `pendidikan_terakhir`, p.id_guru FROM `guru` g join 
        `nilai` p on g.id_guru = p.id_guru WHERE p.id_periode = ?  GROUP BY p.id_guru";
        return $this->db->query($query, $tahun)->result();
    }

    public function countdatanilai($tahun)
    {
        // return $this->db->get_where('nilai', ['id_periode' => $tahun])->result_array();
        $query = "SELECT `id_nilai`, `nama_guru`, `nip`, p.id_guru FROM `guru` g join 
        `nilai` p on g.id_guru = p.id_guru WHERE p.id_periode = ?  GROUP BY p.id_guru";
        return $this->db->query($query, $tahun)->num_rows();
    }
...
public function insertdata($data)
{
    ...
    public function get_Guru($tahun, $admin)
    {
        // return $this->db->get_where('nilai', ['id_periode' => $tahun])->result_array();
        $query = "SELECT `id_nilai`, `nama_guru`, `nip`, p.id_guru FROM `guru` g join 
        `nilai` p on g.id_guru = p.id_guru WHERE p.id_periode = ? AND g.id_admin = ?  GROUP BY p.id_guru";
        return $this->db->query($query, array('id_periode'=>$tahun, 'id_admin'=>$admin))->result_array();
    }

    public function get_unratedGuru($admin)
    {
        return $this->db->get_where('guru', array('id_admin'=>$admin))->result_array();
    }

    public function get_unratedGuruById($id)
    {
         $this->db->select('*');
      $this->db->from('guru');
    ...
    public function get_unratedGuruById2($id)
    {
         $this->db->select('*');
      $this->db->from('guru');
       $this->db->where('id_guru', $id);  
    ...
    public function get_oneperiod($now)
    {
         $this->db->select('*');
      $this->db->from('periode');
       $this->db->where('id_periode', $now);  
      $query = $this->db->get()->row_array();
      return $query;
    }
    ...
    public function get_AllKriteria()
    {
     ...
     public function get_AllKriteriaAndSub()
    {
        $this->db->select('*');
   ...
   public function countdataguru($id)
    {
        return $this->db->get_where('nilai', array('id_guru'=>$id))->num_rows();
    }
    ...
    public function tambah($data,$table){
     ...
     public function tambah_pkg($datar,$table){
        $this->db->insert($table,$data);
    }
    ...
    public function get_NilaiGuru($id, $tahun)
    {
     ...
     public function get_pkg($id, $tahun)
    {
         $this->db->select('*');
      $this->db->from('hasil_pkg');
       $this->db->where('id_guru', $id);  
          $this->db->where('id_guru', $id);  
      $query = $this->db->get()->row_array();
      return $query;
